(function () {
  ...
  const NOTIFICATION_API = "https://aldrich-energy-htx9.vercel.app/api/mecoc-email-notification"; // ← Your Next.js URL
  const WEBSITE_NAME = "MECOC EXPO"; // ← change this per website

  const css = `
    #_cb_launcher{position:fixed;bottom:28px;right:28px;width:56px;height:56px;border-radius:50%;background:#005F55;border:none;cursor:pointer;display:flex;align-items:center;justify-content:center;box-shadow:0 4px 20px rgba(0,0,0,0.2);z-index:99999;transition:transform .2s;}
    #_cb_launcher:hover{transform:scale(1.07);}
    #_cb_launcher::before{content:'';position:absolute;width:56px;height:56px;border-radius:50%;border:2px solid #005F55;animation:_cb_pulse 2s ease-out infinite;}
    ...
    ._cb_header{padding:16px 18px;background:#005F55;display:flex;align-items:center;gap:12px;}
    ...
    ._cb_user{background:#005F55;color:#fff;border-bottom-right-radius:4px;align-self:flex-end;}
    ._cb_qr{display:flex;flex-wrap:wrap;gap:6px;margin-top:4px;animation:_cb_up .25s ease;}
    ._cb_qrbtn{padding:7px 12px;border-radius:20px;border:1.5px solid #005F55;background:#fff;font-size:12px;color:#111;cursor:pointer;transition:all .15s;font-family:inherit;white-space:nowrap;font-weight:500;}
    ._cb_qrbtn:hover{background:#005F55;color:#fff;}
    ...
    ._cb_emailcard{background:#fafafa;border:1.5px solid #005F55;border-radius:14px;padding:14px;animation:_cb_up .25s ease;align-self:flex-start;width:88%;}
    ...
    ._cb_einput:focus{border-color:#005F55;}
    ._cb_esubmit{padding:10px 13px;background:#005F55;color:#fff;border:none;border-radius:8px;font-size:13px;cursor:pointer;font-family:inherit;font-weight:600;transition:background .15s;white-space:nowrap;flex-shrink:0;}
    ._cb_esubmit:hover{background:#8E3914;}
    ...
    #_cb_input:focus{border-color:#005F55;background:#fff;}
    #_cb_input::placeholder{color:#bbb;}
    #_cb_input:disabled{opacity:0.4;cursor:not-allowed;}
    #_cb_sendbtn{width:36px;height:36px;border-radius:50%;background:#005F55;border:none;cursor:pointer;display:flex;align-items:center;justify-content:center;flex-shrink:0;transition:background .15s;}
    #_cb_sendbtn:hover{background:#58595B;}
    ._cb_link{color:#005F55;text-decoration:underline;font-weight:500;display:inline-block;margin-top:6px;}
  ...
  win.innerHTML = `
    <div class="_cb_header">
      <div class="_cb_avatar">
        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="#005F55" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="11" width="18" height="10" rx="2"/><circle cx="12" cy="5" r="2"/><path d="M12 7v4"/><line x1="8" y1="16" x2="8" y2="16"/><line x1="16" y1="16" x2="16" y2="16"/></svg>
      </div>
      <div>
        <div class="_cb_hname">MECOC EXPO 2027 Assistant</div>
    ...
    <div class="_cb_powered">MECOC EXPO 2027 · Smart Assistant</div>
  `;
  document.body.appendChild(win);

  const FAQ = {
    about: "The Middle East Metallurgy Corrosion & Coatings expo (MECOC) gathers technical experts from all over the world to discuss the latest technology related to metallurgy, corrosion & coatings for various industries. The forum enhances communication and knowledge exchange between major stakeholders.\n\nFeatured Conferences:\n• NON-METALLIC MATERIALS CONFERENCE\n• FUTURE STEEL CONFERENCE\n• CORROSION & COATINGS CONFERENCE",
    when: "📅 13-14 APRIL 2027\n📍 Location: Abu Dhabi, UAE\n🎯 Focus: Middle East Metallurgy Corrosion & Coatings Expo",
    register: {
    ...
    sponsor: {
      ...
      link: "register@mecocexpo.com",
      url: "mailto:register@mecocexpo.com"
    }
  };

  const MAIN_QRS = ["What is MECOC EXPO?", "Event Date & Venue", "How to Register?", "Sponsorship Details"];
  ...
  async function submitEmail() {
    ...
    setTimeout(() => {
      removeTyping();
      addBot("🎉 Thank you! Your email has been saved.\n\nWe'll keep you updated on MECOC EXPO 2027. How can I help you today?", MAIN_QRS);
    }, 800);
  }
  ...
  function handleInput(text) {
    ...
    setTimeout(() => {
      ...
      if (t.match(/^(hi|hello|hey|greetings)/)) {
      ...
      } else if (t.includes('what is') || t.includes('about') || t.includes('mecoc expo')) {
        ...
        addBot("You're welcome! 😊 See you at MECOC EXPO 2027!", MAIN_QRS);
      } else if (t.includes('bye') || t.includes('goodbye')) {
        addBot("Goodbye! 👋 See you at MECOC EXPO 2027 in Abu Dhabi. Feel free to come back anytime!", []);
      } else {
        addBot("I'm here to help with MECOC EXPO 2027. Please choose a topic below:", MAIN_QRS);
      }
    }, 700);
  }
